...
import com.gyanbooster.model.PaymentModel;
...
public class PayMentGateWay extends Activity implements Observer {
...
//    String salt="ZhraT96O"; // live
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (extras != null) {
            ...
            getRechargeAmt = extras.getString("RECHARGE_AMT");
        }
...
//                    onBackPressed();
                    ...
                    intent.putExtra(PayData.PAYDATA, paymentResponse.getPay());
                    startActivity(intent);
                } else {
                    showCenteredToast(this, paymentResponse.getMessage());
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
            ...
            mHandler.post(new Runnable() {
                public void run() {
                    mHandler = null;
	                    ...
	                    finish();*/
                    // new PostRechargeData().execute();
                    Intent intent = new Intent(PayMentGateWay.this, DrawerBaseActivity.class);
                    intent.putExtra("test", getFirstName);
                    startActivity(intent);

                    Toast.makeText(getApplicationContext(), "Successfully payment", Toast.LENGTH_LONG).show();
                    paymentModel.paymentApi(PayMentGateWay.this, course, txnid, "1", "0", "42", "");
